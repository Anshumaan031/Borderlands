...
#include <image_transport/image_transport.h>  // provides transparent support for transporting images in low-bandwidth compressed formats.
#include <cv_bridge/cv_bridge.h>  //Image message class that is interoperable with sensor_msgs/Image but uses a more convenient cv::Mat representation for the image data. 
...
{
    private:
        rclpp::NodeHandle nh_; 
        image_transport::ImageTransport it_; 
        image_transport::Subscriber image_sub_;
        image_transport::Publisher image_pub_; 
        rclpp::Subscriber sub;
        float data[5];
...
{
  rclpp::init(argc, argv, "Rectangle_Detector");
  Drawer ic;
  rclpp::spin();
  return 0;
}
